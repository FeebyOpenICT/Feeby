...
class PostController:
    ...
    async def get_posts(self, current_active_user: UserModel = Depends(get_current_active_user)):
        """Get all posts from self from database

        Allowed roles:
        - All
        """
        result = PostService.get_posts_from_user_by_id(
            user_id=current_active_user.id, db=self.db)
    ...
    async def get_post_by_id(self, post_id: int, current_active_user: UserModel = Depends(get_current_active_user)):
        ...
        - All
        """
        post = PostService.get_complete_post_with_access_or_fail(requesting_user_id=current_active_user.id,
