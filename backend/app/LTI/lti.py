...
async def launch(
    ...
    if lis_person_name_full == None or custom_canvas_user_id == None or custom_canvas_user_login_id == None:
        raise LTILaunchException(
            "The Feeby lti tool privacy setting must be set to public")
    ...
    if oauth_consumer_key != CLIENT_ID:
        raise LTILaunchException(
            "oauth_consumer_key does not match the CLIENT_ID")
    ...
    if oauth_signature_method != 'HMAC-SHA1':
        raise LTILaunchException(
            "oauth_signature_method does not match 'HMAC-SHA1'")
    ...
    if cached_oauth_nonce:
        if oauth_nonce == cached_oauth_nonce:
            raise LTILaunchException(
                "oauth_nonce signature has already been made in the last five minutes")

    redis_client.setex(name=oauth_timestamp,
                       time=five_minutes, value=oauth_nonce)
    ...
    # See second canvas link
    signing_key = f"{urllib.parse.quote(CLIENT_SECRET, '')}&"

    hashed = hmac.new(key=bytes(signing_key, 'utf-8'),
                      msg=bytes(signature_base, 'utf-8'), digestmod=sha1)
    signature = str(base64.b64encode(hashed.digest()), 'utf-8').rstrip('\n')

    if oauth_signature != signature:
        raise LTILaunchException(
            f"oauth_signature: {oauth_signature} does not match calculated signature: {signature}")
    ...
    try:
        ...
        raise LTILaunchException(
            f"Could not convert custom_canvas_user_id: {custom_canvas_user_id}, type: {type(custom_canvas_user_id).__name__} to type: int")
    ...
    if jwt == None:
        ...
        redir_to_auth_response.set_cookie(
            "jwt", jwt_token.encoded_token, max_age=2147483647, httponly=False, samesite="None", secure=True)
        ...
        if jwt_token.canvas_id != custom_canvas_user_id_int or jwt_token.refresh_token == None:
            ...
            redir_to_auth_response.set_cookie(
                "jwt", new_jwt_token.encoded_token, max_age=2147483647, httponly=False, samesite="None", secure=True)
            return redir_to_auth_response
